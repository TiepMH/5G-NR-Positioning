...
for idx = 1:n_gNBs
    ...
    contribution_from_gNB_i = [zeros(delay_LOS_in_samples, 1); 
                               txWaveform{idx}; ...
                               zeros(delay_max - delay_LOS_in_samples, 1)
                               ];

    % Transmission through a SISO channel under the impact of multipath fading
    channel_i = channels{idx};
    contribution_from_gNB_i = sqrt(SNR_gNB_i)*channel_i(contribution_from_gNB_i);
