...
% Let's consider the 1-st gNB
delay_in_samples = delays_in_samples(1);

% Take the time delay into account
txWaveform = [zeros(delay_in_samples, n_TxAnt); 
              txWaveform; ...
              zeros(MaximumChannelDelay - delay_in_samples, n_TxAnt)
              ];

% Transmission through a MIMO channel under the impact of multipath fading
rxWaveform_at_gNB1 = channel(txWaveform);
...
disp(['Actual delay (in samples) between the UE and the 1-st gNB: ', num2str(delay_in_samples)]);
